...
TEST_LINES = [
    {
        "position": i,
        "line": f"This is test line number {i + 1} that needs to be translated to Hungarian.",
    }
...
TEST_LINES = [
    {"position": i, "line": VARIED_LINES[i % len(VARIED_LINES)]} for i in range(TOTAL_LINES)
]
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "-X",
                "POST",
                "http://localhost:8765/api/v1/jobs/translate/content",
                "-H",
                ...
                json.dumps(
                    {
                        ...
                        "targetLanguage": "hu",
                        "lines": TEST_LINES,
                        "config": {"apiKey": API_KEY, "model": model_id, "temperature": 0.3},
                    }
                ),
            ],
            capture_output=True,
                ...
                if status == "completed":
                    ...
                    if job_data.get("result", {}).get("lines"):
                        ...
                        return {
                            ...
                            "status": "victory"
                            if hungarian_count >= HUNGARIAN_THRESHOLD
                            else "partial",
...
def battle_round(round_num):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = {
            executor.submit(test_model_round, m[0], m[1], m[2], round_num): m for m in CONTESTANTS
        }
        ...
        for future in concurrent.futures.as_completed(futures, timeout=MAX_ROUND_TIME + 30):
            try:
                ...
                if result.get("status") == "victory":
                    print(
                        f"✅ {result['name']}: {result['hungarian_count']}/{result['total_lines']} HU ({result['success_rate']:.0f}%) in {result['total_time']:.1f}s"
                    )
                elif result.get("status") == "partial":
                    print(
                        f"⚠️  {result['name']}: {result['hungarian_count']}/{result['total_lines']} HU ({result['success_rate']:.0f}%) in {result['total_time']:.1f}s"
                    )
                else:
...
def main():
    ...
    print(
        f"Threshold: {HUNGARIAN_THRESHOLD}/{TOTAL_LINES} ({THRESHOLD_HUNGARIAN_PERCENT}%) Hungarian"
    )
